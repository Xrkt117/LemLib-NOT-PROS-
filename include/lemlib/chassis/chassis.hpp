...
class OdomSensors {
    public:
         ...
         * @b Example
         * @code {.cpp}
         * rotation vertical_rotation(1); // rotation sensor on port 1
         * inertial imu(2); // IMU on port 2
        ...
        OdomSensors(TrackingWheel* vertical1, TrackingWheel* vertical2, TrackingWheel* horizontal1,
                    TrackingWheel* horizontal2, vex::inertial* imu);
...
class Drivetrain {
    public:
         ...
         * @code {.cpp}
         * // drive motors
         * motor lF(-5, ratio6_1); // left front motor. port 5, reversed
         * motor lM(4, ratio6_1); // left middle motor. port 4
         * motor lB(-3, ratio6_1); // left back motor. port 3, reversed
         * motor rF(6, ratio6_1); // right front motor. port 6
         * motor rM(-9, ratio6_1); // right middle motor. port 9, reversed
         * motor rB(7, ratio6_1); // right back motor. port 7
         *
         * // motor groups
         * motor_group leftMotors({lF, lM, lB}); // left motor group
         * motor_group rightMotors({rF, rM, rB}); // right motor group
         *
         ...
         * @endcode
         */
        Drivetrain(vex::motor_group* leftMotors, vex::motor_group* rightMotors, float trackWidth, float wheelDiameter,
                   float rpm, float horizontalDrift, float cartridgeRpm = 600);
...
constexpr AngularDirection clockwise = AngularDirection::CW_CLOCKWISE;
constexpr AngularDirection counterClockwise = AngularDirection::CCW_COUNTERCLOCKWISE;
constexpr AngularDirection shortestTurn = AngularDirection::AUTO;
...
constexpr DriveSide leftSide = DriveSide::LEFT;
constexpr DriveSide rightSide = DriveSide::RIGHT;
...
class Chassis {
    public:
         ...
         *         int leftY = controller.get_analog(Axis3.position());
         *         int rightY = controller.get_analog(Axis2.position());
         ...
         *         // get joystick positions
         *         int leftY = controller.get_analog(Axis3.position());
         *         int rightX = controller.get_analog(Axis1.position());
