...
std::vector<lemlib::Pose> getData(const asset& path) {
    ...
    for (std::string line : dataLines) {
        ...
        pathPoint.x = atof(pointInput.at(0).c_str()); // x position
        pathPoint.y = atof(pointInput.at(1).c_str()); // y position
        pathPoint.theta = atof(pointInput.at(2).c_str()); // velocity
...
void lemlib::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    if (async) {
        motionTask.start([this, path, lookahead, timeout, forwards]() {
    ...
    int closestPoint;
    float prevVel = 0;
    uint32_t compState = platform::competitionStatus();
    distTraveled = 0;

    // loop until the robot is within the end tolerance
    for (int i = 0; i < timeout / 10 && platform::competitionStatus() == compState && this->motionRunning; i++) {
        ...
        // move the drivetrain
        if (forwards) {
            platform::drive(drivetrain.leftMotors, targetLeftVel);
            platform::drive(drivetrain.rightMotors, targetRightVel);
        } else {
            platform::drive(drivetrain.leftMotors, -targetRightVel);
            platform::drive(drivetrain.rightMotors, -targetLeftVel);
        }
