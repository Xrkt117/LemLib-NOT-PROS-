...
void Chassis::tank(int left, int right, bool disableDriveCurve) {
    if (disableDriveCurve) {
        platform::drive(drivetrain.leftMotors, left);
        platform::drive(drivetrain.rightMotors, right);
    } else {
        platform::drive(drivetrain.leftMotors, throttleCurve->curve(left));
        platform::drive(drivetrain.rightMotors, throttleCurve->curve(right));
    }
}
